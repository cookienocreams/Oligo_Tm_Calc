...
def salt_equations(primertm, primergc, primerlength, mg, mon):
    ...
    a, b, c, d, e, f, g = 3.92e-5, -9.11e-6, 6.06e-5, 1.42e-5, -4.82e-4, 5.65e-4, 8.31e-5 #Slightly adjusted constants from the paper

    salt1 = (1 / (primertm + 273.15)) + ((4.59e-5 * (primergc / 100)) - 2.90e-5) * math.log(mon) + 8.81e-6 * (math.log(mon)) ** 2
    
    salt2 = inverse_primer1_corr = (1 / (primertm + 273.15)) + a + (b * math.log(mg)) + ((primergc / 100) * (c + d * math.log(mg))) + (1 / (2.0 * (primerlength - 1))) * (e + f * math.log(mg) + g * math.log(mg) ** 2)
