...
#Captures the first and last base in each primer
...
AA_h_p1 = AA_delta_h * sum([1.0 for base in duo_primer1 if base in AA])
AC_h_p1 = AC_delta_h * sum([1.0 for base in duo_primer1 if base in AC])
AG_h_p1 = AG_delta_h * sum([1.0 for base in duo_primer1 if base in AG])
AT_h_p1 = AT_delta_h * sum([1.0 for base in duo_primer1 if base in AT])
CA_h_p1 = CA_delta_h * sum([1.0 for base in duo_primer1 if base in CA])
CC_h_p1 = CC_delta_h * sum([1.0 for base in duo_primer1 if base in CC])
CG_h_p1 = CG_delta_h * sum([1.0 for base in duo_primer1 if base in CG])
GA_h_p1 = GA_delta_h * sum([1.0 for base in duo_primer1 if base in GA])
GC_h_p1 = GC_delta_h * sum([1.0 for base in duo_primer1 if base in GC])
TA_h_p1 = TA_delta_h * sum([1.0 for base in duo_primer1 if base in TA])

#Calculation of delta S total for each NN pair in primer 1
AA_s_p1 = AA_delta_s * sum([1.0 for base in duo_primer1 if base in AA])
AC_s_p1 = AC_delta_s * sum([1.0 for base in duo_primer1 if base in AC])
AG_s_p1 = AG_delta_s * sum([1.0 for base in duo_primer1 if base in AG])
AT_s_p1 = AT_delta_s * sum([1.0 for base in duo_primer1 if base in AT])
CA_s_p1 = CA_delta_s * sum([1.0 for base in duo_primer1 if base in CA])
CC_s_p1 = CC_delta_s * sum([1.0 for base in duo_primer1 if base in CC])
CG_s_p1 = CG_delta_s * sum([1.0 for base in duo_primer1 if base in CG])
GA_s_p1 = GA_delta_s * sum([1.0 for base in duo_primer1 if base in GA])
GC_s_p1 = GC_delta_s * sum([1.0 for base in duo_primer1 if base in GC])
TA_s_p1 = TA_delta_s * sum([1.0 for base in duo_primer1 if base in TA])

#Calculation of delta H total for each NN pair in primer 2
AA_h_p2 = AA_delta_h * sum([1.0 for base in duo_primer2 if base in AA])
AC_h_p2 = AC_delta_h * sum([1.0 for base in duo_primer2 if base in AC])
AG_h_p2 = AG_delta_h * sum([1.0 for base in duo_primer2 if base in AG])
AT_h_p2 = AT_delta_h * sum([1.0 for base in duo_primer2 if base in AT])
CA_h_p2 = CA_delta_h * sum([1.0 for base in duo_primer2 if base in CA])
CC_h_p2 = CC_delta_h * sum([1.0 for base in duo_primer2 if base in CC])
CG_h_p2 = CG_delta_h * sum([1.0 for base in duo_primer2 if base in CG])
GA_h_p2 = GA_delta_h * sum([1.0 for base in duo_primer2 if base in GA])
GC_h_p2 = GC_delta_h * sum([1.0 for base in duo_primer2 if base in GC])
TA_h_p2 = TA_delta_h * sum([1.0 for base in duo_primer2 if base in TA])

#Calculation of delta S total for each NN pair in primer 2
AA_s_p2 = AA_delta_s * sum([1.0 for base in duo_primer2 if base in AA])
AC_s_p2 = AC_delta_s * sum([1.0 for base in duo_primer2 if base in AC])
AG_s_p2 = AG_delta_s * sum([1.0 for base in duo_primer2 if base in AG])
AT_s_p2 = AT_delta_s * sum([1.0 for base in duo_primer2 if base in AT])
CA_s_p2 = CA_delta_s * sum([1.0 for base in duo_primer2 if base in CA])
CC_s_p2 = CC_delta_s * sum([1.0 for base in duo_primer2 if base in CC])
CG_s_p2 = CG_delta_s * sum([1.0 for base in duo_primer2 if base in CG])
GA_s_p2 = GA_delta_s * sum([1.0 for base in duo_primer2 if base in GA])
GC_s_p2 = GC_delta_s * sum([1.0 for base in duo_primer2 if base in GC])
TA_s_p2 = TA_delta_s * sum([1.0 for base in duo_primer2 if base in TA])
...
AA_NN_sum = AA_delta_g * sum([1.0 for base in duo_sequence if base in AA])
AC_NN_sum = AC_delta_g * sum([1.0 for base in duo_sequence if base in AC])
AG_NN_sum = AG_delta_g * sum([1.0 for base in duo_sequence if base in AG])
AT_NN_sum = AT_delta_g * sum([1.0 for base in duo_sequence if base in AT])
CA_NN_sum = CA_delta_g * sum([1.0 for base in duo_sequence if base in CA])
CC_NN_sum = CC_delta_g * sum([1.0 for base in duo_sequence if base in CC])
CG_NN_sum = CG_delta_g * sum([1.0 for base in duo_sequence if base in CG])
GA_NN_sum = GA_delta_g * sum([1.0 for base in duo_sequence if base in GA]) 
GC_NN_sum = GC_delta_g * sum([1.0 for base in duo_sequence if base in GC])
TA_NN_sum = TA_delta_g * sum([1.0 for base in duo_sequence if base in TA])

#Calculation of delta H total for each NN in the input sequence
AA_h_sum = AA_delta_h * sum([1.0 for base in duo_sequence if base in AA])
AC_h_sum = AC_delta_h * sum([1.0 for base in duo_sequence if base in AC])
AG_h_sum = AG_delta_h * sum([1.0 for base in duo_sequence if base in AG])
AT_h_sum = AT_delta_h * sum([1.0 for base in duo_sequence if base in AT])
CA_h_sum = CA_delta_h * sum([1.0 for base in duo_sequence if base in CA])
CC_h_sum = CC_delta_h * sum([1.0 for base in duo_sequence if base in CC])
CG_h_sum = CG_delta_h * sum([1.0 for base in duo_sequence if base in CG])
GA_h_sum = GA_delta_h * sum([1.0 for base in duo_sequence if base in GA]) 
GC_h_sum = GC_delta_h * sum([1.0 for base in duo_sequence if base in GC])
TA_h_sum = TA_delta_h * sum([1.0 for base in duo_sequence if base in TA])

#Calculation of delta S total for each NN in the input sequence
AA_s_sum = AA_delta_s * sum([1.0 for base in duo_sequence if base in AA])
AC_s_sum = AC_delta_s * sum([1.0 for base in duo_sequence if base in AC])
AG_s_sum = AG_delta_s * sum([1.0 for base in duo_sequence if base in AG])
AT_s_sum = AT_delta_s * sum([1.0 for base in duo_sequence if base in AT])
CA_s_sum = CA_delta_s * sum([1.0 for base in duo_sequence if base in CA])
CC_s_sum = CC_delta_s * sum([1.0 for base in duo_sequence if base in CC])
CG_s_sum = CG_delta_s * sum([1.0 for base in duo_sequence if base in CG])
GA_s_sum = GA_delta_s * sum([1.0 for base in duo_sequence if base in GA]) 
GC_s_sum = GC_delta_s * sum([1.0 for base in duo_sequence if base in GC])
TA_s_sum = TA_delta_s * sum([1.0 for base in duo_sequence if base in TA])
...
def seq_terminal_comp(initial_base, terminal_base, sum_NN_delta_h, sum_NN_delta_s):
    ...
    for base in initial_base:   
        if base == 'A' or 'T':

            sum_NN_delta_h += AT_h
            sum_NN_delta_s += AT_s

            return sum_NN_delta_h, sum_NN_delta_s
...
term_h, term_s = seq_terminal_comp(initial_base, terminal_base, sum_NN_delta_h, sum_NN_delta_s)
...
melting_temperature = (1000 * term_h) / (term_s + (gas_constant_cal * (math.log(oligo_c)))) - 273.15

#Determines the melting temperature of the primers
primer1_gc = get_primer1_gc(primer1)
primer2_gc = get_primer2_gc(primer2)

primer1_melting_temperature = (1000 * p1_term_h) / (p1_term_s + (gas_constant_cal * (math.log(oligo_c)))) - 273.15
primer2_melting_temperature = (1000 * p2_term_h) / (p1_term_s + (gas_constant_cal * (math.log(oligo_c)))) - 273.15
