...
def p1_terminal_comp(p1_initial_base, p1_terminal_base, sum_p1_delta_h, sum_p1_delta_s):
    ...
    AT_h, AT_s, GC_h, GC_s = 2.4, 5.7, .45, -1.7
    dh, ds = 0, 0
...
def seq_terminal_comp(initial_base, terminal_base, sum_NN_delta_h, sum_NN_delta_s):
    ...
    dh_init = sum([dh + AT_h if initial_base in ['A', 'T'] else dh + GC_h])
    ds_init = sum([ds + AT_s if initial_base in ['A', 'T'] else ds + GC_s])

    dh_term = sum([dh + AT_h if terminal_base in ['A', 'T'] else dh + GC_h])
    ds_term = sum([ds + AT_s if terminal_base in ['A', 'T'] else ds + GC_s])
...
melting_temperature = (1000 * term_h) / (term_s + (gas_constant * (math.log(template_c)))) - 273.15
...
primer2_melting_temperature = (1000 * p2_term_h) / (p1_term_s + (gas_constant * (math.log(oligo_c)))) - 273.15
